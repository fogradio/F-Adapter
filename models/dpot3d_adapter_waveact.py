...
class Adapter(nn.Module):
    ...
    def __init__(self, in_features, bottleneck_dim, activation='gelu', scale=1.0, init_option='zero', use_waveact=False):
        super().__init__()
        ...
        # ParameterChoiceActivation function
        if use_waveact:
            self.act = WaveAct()
        else:
...
class AFNO3DAdapter(nn.Module):
    def __init__(
        ...
        # Add adapter modules, UseWaveAct
        # Add input and output adapter for each block
        self.adapters_in = nn.ModuleList([
            Adapter(self.block_size, adapter_dim, activation=act, scale=adapter_scale, use_waveact=True)
            for _ in range(num_blocks)
        ])
        
        self.adapters_mid = nn.ModuleList([
            Adapter(self.block_size * self.hidden_size_factor, adapter_dim, activation=act, scale=adapter_scale, use_waveact=True)
            for _ in range(num_blocks)
        ])
