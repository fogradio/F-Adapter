...
from models.fftKAN import NaiveFourierKANLayer
...
class Adapter(nn.Module):
    """Adapter module implementation using FourierKAN"""
    def __init__(self, in_features, bottleneck_dim, activation='gelu', scale=1.0, 
                 gridsize=300, addbias=True, smooth_initialization=True):
        super().__init__()
        self.in_features = in_features
        self.bottleneck_dim = bottleneck_dim
        self.scale = scale
        
        # Down projection layer: use standard linear layer
        self.down_proj = nn.Linear(in_features, bottleneck_dim)
        
        # Activation function
        self.act = ACTIVATION[activation]
        
        # Up projection layer: use NaiveFourierKANLayer to replace ChebyKANLayer
        self.up_proj = NaiveFourierKANLayer(
            inputdim=bottleneck_dim, 
            outdim=in_features, 
            gridsize=gridsize,
            addbias=addbias,
    ...
    def _init_weights(self):
        """InitializeWeight"""
        # Down projection layerUsekaimingInitialize
        nn.init.kaiming_uniform_(self.down_proj.weight, a=math.sqrt(5))
        if self.down_proj.bias is not None:
            nn.init.zeros_(self.down_proj.bias)
        
        # FourierKAN initialization is already processed internally
    
    def forward(self, x):
        """Forward pass: Input -> Down projection -> Activation -> FourierKAN up projection -> Scale -> Return"""
        identity = x
...
# DefineFrequency domainAdapter，ForAFNO3D
class AFNO3DAdapter(nn.Module):
    def __init__(self, width=32, num_blocks=8, channel_first=False, sparsity_threshold=0.01,
                 ...
                 act='gelu', adapter_dim=32, adapter_scale=1.0, gridsize=32, addbias=True, smooth_initialization=True, share_adapters=True):
        ...
        if share_adapters:
            # All blocks share one set of adapters
            shared_in_adapter = Adapter(self.block_size, adapter_dim, activation=act, 
                                      scale=adapter_scale, gridsize=gridsize, 
                                      addbias=addbias, smooth_initialization=smooth_initialization)
            shared_mid_adapter = Adapter(self.block_size * self.hidden_size_factor, adapter_dim, 
                                       activation=act, scale=adapter_scale, gridsize=gridsize, 
                                       addbias=addbias, smooth_initialization=smooth_initialization)
            shared_out_adapter = Adapter(self.block_size, adapter_dim, activation=act, 
                                       scale=adapter_scale, gridsize=gridsize, 
                                       addbias=addbias, smooth_initialization=smooth_initialization)
            
            self.adapters_in = nn.ModuleList([shared_in_adapter for _ in range(num_blocks)])
            self.adapters_mid = nn.ModuleList([shared_mid_adapter for _ in range(num_blocks)])
            self.adapters_out = nn.ModuleList([shared_out_adapter for _ in range(num_blocks)])
        else:
            # Original code: each block has independent adapters
            self.adapters_in = nn.ModuleList([
                Adapter(self.block_size, adapter_dim, activation=act, scale=adapter_scale, gridsize=gridsize, addbias=addbias, smooth_initialization=smooth_initialization)
                for _ in range(num_blocks)
            ])
            
            self.adapters_mid = nn.ModuleList([
                Adapter(self.block_size * self.hidden_size_factor, adapter_dim, activation=act, 
                       scale=adapter_scale, gridsize=gridsize, addbias=addbias, smooth_initialization=smooth_initialization)
                for _ in range(num_blocks)
            ])
    ...
    def forward(self, x, spatial_size=None):
        ...
        for block_idx in range(self.num_blocks):
            ...
            # ConvertDataFlattenToApplyAdapter
            ...
            # Store converted results into output container
        ...
        # Convert real/imag back to complex and perform IFFT
...
class PatchEmbedAdapter(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, out_dim=128, act='gelu',
                 adapter_dim=32, adapter_scale=1.0, gridsize=300, addbias=True, smooth_initialization=True):
        super().__init__()
        ...
        # Modified channel adapter implementation
        self.channel_adapter1 = nn.Sequential(
            nn.Conv3d(embed_dim, adapter_dim, kernel_size=1, stride=1),
            self.act,
            nn.Conv3d(adapter_dim, embed_dim, kernel_size=1, stride=1)  # ReplaceNaiveFourierKANLayer
        )
        
        self.channel_adapter2 = nn.Sequential(
            nn.Conv3d(out_dim, adapter_dim, kernel_size=1, stride=1),
            self.act,
            nn.Conv3d(adapter_dim, out_dim, kernel_size=1, stride=1)  # ReplaceNaiveFourierKANLayer
        )
        
        # AdapterScaling factor
        self.adapter_scale = adapter_scale
        
        # Initialize
        self._init_adapter_weights(gridsize, addbias, smooth_initialization)
    
    def _init_adapter_weights(self, gridsize, addbias, smooth_initialization):
        """Initialize adapter weights close to zero, ensuring minimal impact on original network during early training"""
    ...
    def forward(self, x):
        ...
        # Apply first Adapter（AddDimensionCheck）
        adapter_out1 = self.channel_adapter1(x1)
        assert x1.shape == adapter_out1.shape, f"Shape mismatch: x1 {x1.shape}, adapter_out1 {adapter_out1.shape}"
        x1 = x1 + adapter_out1 * self.adapter_scale
        ...
        # Apply second Adapter（AddDimensionCheck）
        adapter_out2 = self.channel_adapter2(x2)
        assert x2.shape == adapter_out2.shape, f"Shape mismatch: x2 {x2.shape}, adapter_out2 {adapter_out2.shape}"
        x = x2 + adapter_out2 * self.adapter_scale
        
        return x

# ModifyBlockClassToUseAFNO3DAdapter
class BlockAdapter(nn.Module):
    def __init__(self, mixing_type='afno', double_skip=True, width=32, n_blocks=4, mlp_ratio=1.,
                 ...
                 adapter_dim=32, adapter_scale=1.0, gridsize=300, addbias=True, smooth_initialization=True, use_adapter=True):
        super().__init__()
        ...
        self.mlp_adapter = nn.Sequential(
            ...
            self.act,
            nn.Conv3d(in_channels=adapter_dim, out_channels=width, kernel_size=1, stride=1)  # ReplaceNaiveFourierKANLayer
        )
        
        # AdapterScaling factor
        self.adapter_scale = adapter_scale
        
        # Initialize adapter close to zero
    ...
    def forward(self, x):
        ...
        # Add assertion to check dimension
        assert mlp_out.shape == adapter_out.shape, f"Shape mismatch: mlp_out {mlp_out.shape}, adapter_out {adapter_out.shape}"
        
        # Add residual and adapter output
        if self.use_adapter:
            x = mlp_out + adapter_out * self.adapter_scale + residual
        else:
            x = mlp_out + residual

        return x

# Complete DPOTNet3D model with all adapter components integrated
class DPOTNet3D(nn.Module):
    def __init__(self, img_size=224, patch_size=16, mixing_type='afno', in_channels=1, out_channels=3, 
                 ...
                 adapter_dim=32, adapter_scale=1.0, gridsize=32, addbias=True, 
                 smooth_initialization=True, share_adapters=True, sparse_adapter_layers=True):
        super().__init__()
        ...
        # Use block with adapter, ButInLayerUse
        self.blocks = nn.ModuleList([
            BlockAdapter(
                ...
                addbias=addbias,
                smooth_initialization=smooth_initialization,
                use_adapter=(i % 3 == 0) if sparse_adapter_layers else True  # Use adapter every 3 layers
        ...
        # ModifyOutput layers AdapterImplementation
        self.out_layer_adapter = nn.Sequential(
            ...
            nn.Conv3d(
                ...
                kernel_size=1, 
                stride=1
            )  # UseConv3dReplaceNaiveFourierKANLayer
        )
        
        # AdapterScaling factor
        self.adapter_scale = adapter_scale
        
        # InitializeOutput layersAdapterZero
        self._init_adapter_weights()
    ...
    def _init_adapter_weights(self):
        """InitializeOutputLayerAdapterWeight"""
    ...
    def forward(self, x):
        ...
        # AdapterOutputLayer（AddshapeCheck）
        out_adapter = self.out_layer_adapter(x)
        
        # AddDimensionCheck
        assert out_orig.shape == out_adapter.shape, \
            f"Shape mismatch: out_orig {out_orig.shape}, out_adapter {out_adapter.shape}"
        
        # MergeOutput
        x = out_orig + out_adapter * self.adapter_scale
        ...
        #print(f"out_orig shape: {out_orig.shape}")Debug
        #print(f"out_adapter shape: {out_adapter.shape}")Debug
...
# Preserve original TimeAggregator and other auxiliary function definitions
...
if __name__ == "__main__":
    ...
    # Add safe global variable
    torch.serialization.add_safe_globals([argparse.Namespace])
    
    load_path = "/NEW_EDS/JJ_Group/zhw/DPOT/model_S.pth"
    net = DPOTNet3D(img_size=64, patch_size=8, in_channels=3, out_channels=3, 
                    in_timesteps=10, embed_dim=1024, n_blocks=8, depth=6,
                    adapter_dim=32, adapter_scale=1.0, gridsize=300, addbias=True, smooth_initialization=True, sparse_adapter_layers=True)

    # Modified loading method
    try:
        state_dict = torch.load(load_path, map_location='cpu', weights_only=False)['model']
        print("Successfully loaded model weights")
    except Exception as e:
        print(f"Error loading model weights: {e}")
        print("Trying alternative loading method...")
        try:
            state_dict = torch.load(load_path, map_location='cpu', weights_only=True)['model']
            print("Successfully loaded model weights with weights_only=True")
        except Exception as e:
    ...
    if state_dict is not None:
        ...
        print("Could not load model weights, skipping forward pass")