...
class Adapter(nn.Module):
    ...
    def _init_weights(self, init_option):
        """InitializeWeight"""
        if init_option == 'zero':
            # ConvertUp projection layerInitialize0, EnsureEarly trainingNo impactOriginal model
            nn.init.zeros_(self.up_proj.weight)
            nn.init.zeros_(self.up_proj.bias)
        elif init_option == 'normal':
            # UseNormal distributionInitialize
    ...
    def forward(self, x):
        """Forward pass：Input->Down projection->activation->Up projection->Scale->Return"""
        # SaveInputForResidual connection
        identity = x
...
class AFNO3DAdapter(nn.Module):
    def __init__(
        ...
        act='gelu',
        adapter_dim=32,  # Adapter BottleneckDimension
        adapter_scale=1.0,
        power=2,  # Dimension Parameter 2
        num_bands=4,  # Frequency bandNumber 4
    ):
        ...
        # SetBottleneckDimensionRange
        self.min_dim = adapter_dim // 2
        self.max_dim = adapter_dim * 2
        self.power = power
    ...
    def forward(self, x, spatial_size=None):
        ...
        for block_idx in range(self.num_blocks):
            ...
            for band_idx in range(self.num_bands):
                ...
                # ApplyFrequency bandFor InputAdapter
                ...
                # ApplyFrequency bandFor IntermediateAdapter
                ...
                # ApplyFrequency bandFor OutputAdapter
...
class PatchEmbedAdapter(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, out_dim=128, act='gelu',
        ...
        # AddChannelAdapter (ForFeatureChannel Adjust)
