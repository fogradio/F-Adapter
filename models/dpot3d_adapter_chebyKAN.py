...
class Adapter(nn.Module):
    """Adapter module implementation using ChebyKANLayer without activation function"""
    def __init__(self, in_features, bottleneck_dim, activation='gelu', scale=1.0, degree=8):
        super().__init__()
        ...
        # Activation function (for initialization only, not used in forward pass)
        self.activation = activation
        
        # Up projection layer：UseChebyKANLayer
        self.up_proj = ChebyKANLayer(input_dim=bottleneck_dim, output_dim=in_features, degree=degree)
    ...
    def _init_weights(self):
        ...
        # ChebyKANLayer coefficients are initialized to 0
        with torch.no_grad():
            nn.init.zeros_(self.up_proj.cheby_coeffs)
    
    def forward(self, x):
        """Forward pass: Input -> Down projection -> ChebyKAN up projection directly (no activation function) -> Scale -> Return"""
        identity = x
        
        # Down projection
        x = self.down_proj(x)
        
        # UseChebyKANUp projection, SkipActivation function
        x = self.up_proj(x)
...
class AFNO3DAdapter(nn.Module):
    def __init__(self, width=32, num_blocks=8, channel_first=False, sparsity_threshold=0.01,
                 ...
                 act='gelu', adapter_dim=32, adapter_scale=1.0, cheby_degree=8):
        ...
        # AddAdapterModule
        # Add input and output adapter for each block
        self.adapters_in = nn.ModuleList([
            Adapter(self.block_size, adapter_dim, activation=act, scale=adapter_scale, degree=cheby_degree)
            for _ in range(num_blocks)
        ])
        
        self.adapters_mid = nn.ModuleList([
            Adapter(self.block_size * self.hidden_size_factor, adapter_dim, activation=act, 
                   scale=adapter_scale, degree=cheby_degree)
            for _ in range(num_blocks)
        ])
...
class PatchEmbedAdapter(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, out_dim=128, act='gelu',
                 adapter_dim=32, adapter_scale=1.0, cheby_degree=8):
        super().__init__()
        ...
        self.channel_adapter1 = nn.Sequential(
            ...
            self.act,
            nn.Conv3d(adapter_dim, embed_dim, kernel_size=1, stride=1)  # ReplaceChebyKANLayer
        )
        
        self.channel_adapter2 = nn.Sequential(
            nn.Conv3d(out_dim, adapter_dim, kernel_size=1, stride=1),
            self.act,
            nn.Conv3d(adapter_dim, out_dim, kernel_size=1, stride=1)  # ReplaceChebyKANLayer
        )
...
class BlockAdapter(nn.Module):
    def __init__(self, mixing_type='afno', double_skip=True, width=32, n_blocks=4, mlp_ratio=1.,
        ...
        self.mlp_adapter = nn.Sequential(
            ...
            self.act,
            nn.Conv3d(in_channels=adapter_dim, out_channels=width, kernel_size=1, stride=1)  # ReplaceChebyKANLayer
        )
...
if __name__ == "__main__":
    ...
    load_path = "/NEW_EDS/zhw/DPOT/model_S.pth"
